...
def search_stocks(query):
    """API endpoint to search stocks by query"""
    try:
        # First try exact match in common stocks
        lookup_name = query.lower()
        if lookup_name in common_stocks:
            ticker = common_stocks[lookup_name]
            data = get_nse_stock_data(ticker)
            if data:
        ...
        # If not found, return mock search results
        results = []
        for stock_name, ticker in common_stocks.items():
            if query.lower() in stock_name.lower() or query.upper() in ticker:
                data = get_nse_stock_data(ticker)
                if data:
                    results.append(data)
